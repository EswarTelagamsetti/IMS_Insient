...
def raise_ticket():
    if request.method == 'POST':
        ...
        cursor.execute("INSERT INTO tickets (title, description, raised_by, assigned_to) VALUES (%s, %s, %s, %s)", (title, description, session['user_id'], assigned_to))
...
def complete_ticket(ticket_id):
    from app import mysql
    cursor = mysql.connection.cursor()
    cursor.execute("UPDATE tickets SET status = 'completed', completed_at = %s WHERE id = %s AND assigned_to = %s", (datetime.now(), ticket_id, session['user_id']))
