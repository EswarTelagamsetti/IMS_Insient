...
def view_activity():
    ...
    # Fetch logs from previous day to next day for full coverage
    ...
    for user_id, entries in user_sessions.items():
        ...
        # Still active after the last log (e.g., available but no OFF)
        if session_start:
            session_start_clipped = max(session_start, datetime.combine(selected_date, datetime.min.time()))
...
def leave_applications():
    ...
    # Mark admin notifications as read for pending applications
    cursor.execute("""
