...
@admin_bp.route('/view_activity', methods=['GET'])
@admin_required
def view_activity():
    from app import mysql
    ...
    for user_id, entries in user_sessions.items():
        ...
        for entry in entries:
            ts = entry['timestamp']
            status = entry['status']
            if status == 1 and session_start is None:
                session_start = ts
    ...
    return render_template('admin/view_activity.html', selected_date=selected_date, activity_data=activity_data)
...
def leave_applications():
    ...
    return render_template('admin/leave_applications.html', leave_applications=leave_applications, branches=branches)
